...
export class OpenApiV3Parser implements IOpenApiParserPlugin{
  ...
  private parseModels(config: OpenApiV3): Observable<{models: Sw2NgxModel[], enums: Sw2NgxEnum[]}>{
    ...
    if(config?.components?.schemas) {
      ...
      modelsDefs.models =  Object.entries(config.components.schemas).map(([name, definition])=>{
        const parserFn = this.parserConfig?.config.value?.parserModelName as (name:string)=> string
          ...
          .map(
            ([propName, propDef]) => {
              ...
              if(Array.isArray(definition.required)){
                isRequired = (definition?.required?.filter(name=> name === propName)|| []).length>0 || false
  ...
  parseServices(config: OpenApiV3, modelsAndEnums: {enums: Sw2NgxEnum[], models: Sw2NgxModel[]}): Observable<Sw2NgxApiDefinition> {
    ...
    if(config.paths){
      const servicesList = Object.entries(config.paths).map(([servicePath, serviceDef])=>{
        return ['get', 'post', 'put', 'delete', 'head', 'options']
          .map(
            (servicePathMethod) => {
              const serviceMethodDef: OperationV3 = serviceDef[servicePathMethod] as OperationV3
              const response = (serviceMethodDef?.responses?.['200']?.['content'] as {[key:string]: Schema})?.['application/json']['schema'] as Schema
              return serviceMethodDef
                ? resolveMethodFn(servicePath, servicePathMethod as MethodType, serviceMethodDef as unknown as Operation, response , this.parserConfig?.config.value as Sw2NgxConfig)
                : null}
          )
      })
        .reduce((acc, cur)=>{
