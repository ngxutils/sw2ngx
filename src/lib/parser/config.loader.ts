...
export class SwaggerConfigLoader {
  ...
  private getByUrl(configPath: string): Observable<any> {
    ...
    fetch(configPath)
      .catch((err) => {
        swaggerConfig.error(
          `[ERROR]: loading swagger.json (Stack Trace: ) ${JSON.stringify(err)}`
        );
      ...
      .catch((err) =>
        swaggerConfig.error(
          `[ERROR]: Parsing swagger json (Stack Trace: ) ${JSON.stringify(err)}`
        )
      );
  ...
  private getByFilePath(configPath: string) {
    ...
    try {
      ...
      swaggerConfig.error(
        `[ERROR]: Load or parsing swagger config (Stack Trace: ) ${JSON.stringify(
          err
        )}`
      );
    }
