...
import { IParserEnum, IParserModel, IParserServiceList } from './interfaces/parser';
...
export default class Generator {
    ...
    constructor(
        config: IGeneratorConfig | null = null
    ) {
        if (config) {
            this.config = config;
        } else {
    ...
    public start() {
        this.getConfig(this.config.config).then((res) => {
            ...
            this.parser.parse(res).then(
                (res: [IParserEnum[], IParserModel[], IParserServiceList]) => {
                                ...
                                if (extend.services.hasOwnProperty(key)) {
                                    if (res[2].hasOwnProperty(key)) {
                                        if (extend.services[key].imports) {
                                            res[2][key].imports.push(...extend.services[key].imports);
                                        }
                                        if (extend.services[key].methods) {
                                            res[2][key].methods.push(...extend.services[key].methods);
                                        }
                                    } else {
                    ...
                    this._printer.print(res[0], res[1], res[2], this.config.out).then(()=> {
                        this._logger.ok('[ SUCCESS ]: Generation API Module Successfull!');
                    }, (reject)=> {
        ...
        const promise = new Promise<any>((resolve, reject) => {
            if (/http(s?)\:\/\/\S/ig.test(conf)) {
                http.get(conf, (res) => {
                    ...
                    res.on('end', () => {
                        try {
                            this.swagger = JSON.parse(rawData);
